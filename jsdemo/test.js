...
function getWeightedAverage(cancleProNum,cancleQuaNum){
    ...
    var creditArray = $('#grid21344342991_data tr').map((index,item)=> +$($(item).find('td')[5]).html());     var gradeArray = $('#grid21344342991_data tr').map((index,item)=> +$($(item).find('td')[8]).html());
    var classTypeArray =$('#grid21344342991_data tr').map((index,item)=> $($(item).find('td')[4]).html());    creditArray =$.makeArray(creditArray);
